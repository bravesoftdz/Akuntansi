...
uses u_cari, u_dm, u_utama, UFungsi;
...
var a:integer;
begin
fungsi.SQLExec(dm.Q_temp,'select IFNULL(count(no_ix),0) as jumlah from tb_jurnal_global where refr=''GJ''',true);
a:= (dm.Q_temp.fieldbyname('jumlah').AsInteger)+1;

  if a<10       then ed_refrensi.Text:= 'GJ-000000'+inttostr(a) else
  if a<100      then ed_refrensi.Text:= 'GJ-00000' +inttostr(a) else
  if a<1000     then ed_refrensi.Text:= 'GJ-0000'  +inttostr(a) else
  if a<10000    then ed_refrensi.Text:= 'GJ-000'   +inttostr(a) else
  if a<100000   then ed_refrensi.Text:= 'GJ-00'    +inttostr(a) else
  if a<1000000  then ed_refrensi.Text:= 'GJ-0'     +inttostr(a);
...
ix_jurnal:= (dm.Q_temp.fieldbyname('jumlah').AsInteger)+1;
  ...
  floattostr(TableView.DataController.GetValue(x,2))+'","'+floattostr(TableView.DataController.GetValue(x,3))+'"),';
  end;
...
ed_refrensi.Text+'","GJ","'+floattostr(tot_debet)+'")',false);
...
procedure TF_GJurnal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_GJurnal.ed_codeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
