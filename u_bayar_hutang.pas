...
uses
  ...
  Dialogs, StdCtrls, sLabel, sEdit, Buttons, sSpeedButton, sButton,
  sCurrEdit, sCurrencyEdit, Grids, Mask, sMaskEdit, sCustomComboEdit,
  sTooledit, ExtCtrls, sPanel, cxStyles, cxCustomData, cxGraphics,
  cxFilter, cxData, cxDataStorage, cxEdit, cxCurrencyEdit, sGroupBox,
...
type
  Tf_bayar_hutang = class(TForm)
    ...
    procedure sg_jurnalSelectCell(Sender: TObject; ACol, ARow: Integer;
    ...
    procedure ed_no_jurnalKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses u_dm,u_cari, u_utama, UFungsi;
...
var a:integer;
begin
fungsi.SQLExec(dm.Q_temp,'select IFNULL(count(no_ix),0) as jumlah from tb_jurnal_global where refr=''PH''',true);
a:= (dm.Q_temp.fieldbyname('jumlah').AsInteger)+1;

  if a<10      then ed_refrensi.Text:= 'PH-000000' +inttostr(a) else
  if a<100     then ed_refrensi.Text:= 'PH-00000'  +inttostr(a) else
  if a<1000    then ed_refrensi.Text:= 'PH-0000'   +inttostr(a) else
  if a<10000   then ed_refrensi.Text:= 'PH-000'    +inttostr(a) else
  if a<100000  then ed_refrensi.Text:= 'PH-00'     +inttostr(a) else
  if a<1000000 then ed_refrensi.Text:= 'PH-0'      +inttostr(a) ;
...
application.CreateForm(tf_cari, f_cari);
fungsi.SQLExec(dm.q_cari,'select kd_kiraan, n_kiraan from tb_kiraan where kd_kiraan like ''11%''',true);
f_cari.clm1.caption:='Kode';
...
procedure Tf_bayar_hutang.sg_jurnalSelectCell(Sender: TObject; ACol,
  ARow: Integer; var CanSelect: Boolean);
begin
if acol<3 then canselect:= false else canselect:=true;
end;
...
application.CreateForm(tf_cari, f_cari);
fungsi.SQLExec(dm.q_cari,'select tanggal, faktur, hutang from vw_hutang where kd_perusahaan="'+
f_utama.sb.Panels[3].Text+'" and supplier="'+kd_supp+'" and status=''belum lunas''',true);
...
jurnal_hutang:= dm.Q_temp.fieldbyname('kd_akun').asstring;
  ...
  +'","'+inttostr(x+1)+'","'+jurnal_hutang+'","'+floattostr(TableView.DataController.GetValue(x,3))+'","'+
  TableView.DataController.GetDisplayText(x,0)+'"),';

  fungsi.SQLExec(dm.Q_exe,'update tb_hutang set dibayar=dibayar+'+
  floattostr(TableView.DataController.GetValue(x,3))+',`update`=date(now()) where faktur="'+
  TableView.DataController.GetDisplayText(x,0)+'"',false);
  end;
  delete(isi_sql,length(isi_sql),1);


fungsi.SQLExec(dm.Q_exe,'insert into tb_jurnal_global(kd_perusahaan,no_ix,tgl,keterangan, '+
'no_refrensi,refr,nilai) values ("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)
+'","'+formatdatetime('yyyy-MM-dd',de_tanggal.Date)+'","'+ed_keterangan.Text+' untuk '+
ed_ket2.Text+'","'+ed_refrensi.Text+'","PH","'+floattostr(dibayar)+'")',false);

  fungsi.SQLExec(dm.Q_exe,'insert into tb_jurnal_rinci(kd_perusahaan,ix_jurnal,no_urut,kd_akun, '+
  'debet,rujukan) values '+isi_sql,False);

fungsi.SQLExec(dm.Q_exe,'insert into tb_jurnal_rinci(kd_perusahaan,ix_jurnal,no_urut,kd_akun, '+
'kredit) values ("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)+'","'+
inttostr(tableview.DataController.RecordCount+1)+'","'+ed_no_jurnal.Text+'","'+floattostr(dibayar)+'")', false);
...
procedure Tf_bayar_hutang.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tf_bayar_hutang.ed_no_jurnalKeyDown(Sender: TObject;
...
begin
  ...
  begin
    if CompareStr(dm.Q_temp.fieldbyname('faktur').AsString,TableView.DataController.GetValue(f,0)) = 0 then
    begin
...
procedure Tf_bayar_hutang.ed_codeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=vk_return then
begin
  ...
  fungsi.sqlExec(dm.Q_temp,'select tanggal, faktur, hutang from vw_hutang where kd_perusahaan="'+
  f_utama.sb.Panels[3].Text+'" and supplier="'+kd_supp+'" and status=''belum lunas'' and faktur ='+quotedstr(ed_code.Text)+'', true);
...
b:= TableView.DataController.GetFocusedRecordIndex;
 ...
 begin
  ...
  begin
  TableView.DataController.SetValue(b, 3, TableView.DataController.GetValue(b, 2)); //nilai
  Exit;
  end;
...
procedure Tf_bayar_hutang.ed_suppKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if Key= vk_return then
  sb_1Click(Sender);
