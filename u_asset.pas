...
fungsi.SQLExec(dm.Q_temp,'select n_kiraan from tb_kiraan where kd_kiraan="'+cb_Asset.Text+'"',true);
...
fungsi.SQLExec(dm.Q_temp,'select n_kiraan from tb_kiraan where kd_kiraan="'+cb_Dep.Text+'"',true);
...
fungsi.SQLExec(dm.Q_temp,'select n_kiraan from tb_kiraan where kd_kiraan="'+cb_Ak_Dep.Text+'"',true);
...
if anyar=true then
begin
  fungsi.SQLExec(dm.Q_Exe,'insert into tb_asset(kd_perusahaan,kd_asset,Nm_asset, '+
  'tgl_perolehan,Q_asset,jumlah_nilai,nilai_residu,umur_ekonomis,kiraan_asset,kiraan_depresiasi, '+
  'kiraan_akum_depresiasi) values ("'+f_utama.sb.Panels[3].Text+'","'+ed_kode.Text+'","'+ed_nama.Text+'","'+
  formatdatetime('yyyy-MM-dd',de_tanggal.Date)+'","'+ce_qty.Text+'","'+ce_nilai.Text+'","'+ce_res.Text+'","'+
  ce_umur.Text+'","'+cb_Asset.Text+'","'+cb_dep.Text+'","'+cb_Ak_Dep.Text+'")',false);
end else
begin
  fungsi.SQLExec(dm.Q_Exe,'update tb_asset set Nm_asset="'+ed_nama.Text+'",tgl_perolehan="'+
  formatdatetime('yyyy-MM-dd',de_tanggal.Date)+'",Q_asset="'+ce_qty.Text+'",jumlah_nilai="'+
  ce_nilai.Text+'",nilai_residu="'+ce_res.Text+'",umur_ekonomis="'+ce_umur.Text+'",kiraan_asset="'+cb_Asset.Text+'",kiraan_depresiasi="'+
  cb_dep.Text+'",kiraan_akum_depresiasi="'+cb_Ak_Dep.Text+'" where kd_perusahaan="'+
  f_utama.sb.Panels[3].Text+'" and kd_asset="'+ed_kode.Text+'"',false);
end;
...
begin
fungsi.SQLExec(dm.Q_temp,'select * from tb_kiraan where kd_kiraan="'+cb_asset.Text+'"',true);
l_kirAsset.Caption:= dm.Q_temp.fieldbyname('n_kiraan').AsString;
end;

procedure TF_asset.cb_depPropertiesChange(Sender: TObject);
begin
fungsi.SQLExec(dm.Q_temp,'select * from tb_kiraan where kd_kiraan="'+cb_dep.Text+'"',true);
l_kirDep.Caption:= dm.Q_temp.fieldbyname('n_kiraan').AsString;
end;

procedure TF_asset.cb_Ak_depPropertiesChange(Sender: TObject);
begin
fungsi.SQLExec(dm.Q_temp,'select * from tb_kiraan where kd_kiraan="'+cb_ak_dep.Text+'"',true);
l_kirAkDep.Caption:= dm.Q_temp.fieldbyname('n_kiraan').AsString;
end;

procedure TF_asset.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
