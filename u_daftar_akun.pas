...
type
  Tf_daftar_akun = class(TForm)
    ...
    procedure t_dataEditKeyDown(Sender: TcxCustomGridTableView; AItem:
      TcxCustomGridTableItem; AEdit: TcxCustomEdit; var Key: Word; Shift: TShiftState);
...
uses
  u_dm, u_utama, u_klasifikasi_akun, UFungsi;
...
procedure Tf_daftar_akun.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tf_daftar_akun.t_dataEditKeyDown(Sender: TcxCustomGridTableView; AItem:
  TcxCustomGridTableItem; AEdit: TcxCustomEdit; var Key: Word; Shift: TShiftState);
begin
...
begin
  if messagedlg('Yakin akan menghapus data ini...', mtconfirmation, [mbyes, mbno],
    0) = mryes then
  begin
    fungsi.SQLExec(dm.Q_Exe, 'delete from tb_kiraan where kd_kiraan=' + dm.Q_kiraan.fieldbyname
      ('kd_kiraan').AsString + '', false);
