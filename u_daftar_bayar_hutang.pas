...
uses
  ...
  Buttons, sSpeedButton, ExtCtrls, sPanel, StdCtrls, sButton;
...
begin
fungsi.SQLExec(Q_bayar_hutang,'select * from tb_jurnal_global where kd_perusahaan= '''+
f_utama.sb.Panels[3].Text+''' and refr=''PH'' and month(tgl)='''+
f_utama.sb.Panels[6].Text+''' and year(tgl)='''+f_utama.sb.Panels[7].Text+'''',true);

fungsi.SQLExec(Q_rinci_bayar_hutang,'select * from _vw_jurnal_rinci  where kd_perusahaan= '''+
f_utama.sb.Panels[3].Text+''' and refr=''PH'' and bulan='''+
...
procedure Tf_daftar_bayar_hutang.FormClose(Sender: TObject;
...
f_utama.sb.Panels[7].Text+'" and refr="PH"',true);
