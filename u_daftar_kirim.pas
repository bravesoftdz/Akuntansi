...
begin
fungsi.SQLExec(Q_kirim,'select *,IF(IFNULL(ix_kirim,0)=0,0,1) as posted from vw_list_kirim where kd_perusahaan= "'+
f_utama.sb.Panels[3].Text+'" and month(tgl_kirim)="'+f_utama.sb.Panels[6].Text+'" and year(tgl_kirim)="'+
f_utama.sb.Panels[7].Text+'" order by tgl_kirim DESC',true);

fungsi.SQLExec(Q_daftar_kirim,'select * from tb_jurnal_global where kd_perusahaan= "'+
f_utama.sb.Panels[3].Text+'" and no_ix="'+Q_kirim.fieldbyname('ix_kirim').AsString+'"',true);

fungsi.SQLExec(Q_rinci_kirim,'select ix_jurnal,no_urut,kd_akun,n_kiraan,debet,kredit from _vw_jurnal_rinci  where kd_perusahaan= '''+
f_utama.sb.Panels[3].Text+''' and ix_jurnal = "'+Q_kirim.fieldbyname('ix_kirim').AsString+'"',true);
...
procedure Tf_daftar_kirim.FormClose(Sender: TObject;
...
procedure Tf_daftar_kirim.gridActiveTabChanged(Sender: TcxCustomGrid;
  ALevel: TcxGridLevel);
begin
...
procedure Tf_daftar_kirim.t_data0FocusedRecordChanged(
...
begin
fungsi.SQLExec(Q_daftar_kirim,'select * from tb_jurnal_global where kd_perusahaan= "'+
f_utama.sb.Panels[3].Text+'" and refr="KR" and month(tgl)="'+
f_utama.sb.Panels[6].Text+'" and year(tgl)="'+f_utama.sb.Panels[7].Text+'" order by tgl DESC',true);

fungsi.SQLExec(Q_rinci_kirim,'select * from _vw_jurnal_rinci  where kd_perusahaan= '''+
f_utama.sb.Panels[3].Text+''' and refr="KR" and bulan='''+
...
f_utama.sb.Panels[7].Text+'" and refr="KR"',true);
