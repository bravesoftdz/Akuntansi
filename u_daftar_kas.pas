...
type
  Tf_daftar_kas = class(TForm)
    ...
    procedure rg_jenisChanging(Sender: TObject; NewIndex: Integer; var
      AllowChange: Boolean);
...
begin
  fungsi.SQLExec(Q_kas, 'select * from tb_jurnal_global where kd_perusahaan= '''
    ...
    dm.Bulan + ''' and year(tgl)=''' + dm.Tahun
    + '''', true);

  fungsi.SQLExec(Q_rinci_kas,
    'select * from vw_jurnal_rinci  where kd_perusahaan= ''' + 
    dm.kd_perusahaan + ''' and ' + j_trans + ' and bulan=''' + dm.Bulan
...
procedure Tf_daftar_kas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tf_daftar_kas.rg_jenisChanging(Sender: TObject; NewIndex: Integer; var
  AllowChange: Boolean);
begin
  case NewIndex of
    0:
