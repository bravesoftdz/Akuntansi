...
uses
  ...
  Dialogs, Grids, DBGrids, StdCtrls, sEdit, sButton, ExtCtrls, DBCtrls,
  sSkinProvider, sLabel, cxStyles, cxCustomData, cxGraphics, cxFilter, cxData,
...
uses
  u_dm, u_utama, u_bayar_piutang, u_bayar_hutang, u_GJurnal, u_Jurnal_Kas,
  U_Login, u_akun_penting, UFungsi;
...
begin
  ...
  case dm.q_cari.FieldCount of
    2:
      begin
        fungsi.SQLExec(dm.q_cari, 'select ' + clm1.DataBinding.FieldName + ',' +
          clm2.DataBinding.FieldName + ' from ' + nm_tabel + ' where ' + kondisi
          + '(' + clm1.DataBinding.FieldName + ' like "%' + ed_cari.Text +
          '%" or ' + clm2.DataBinding.FieldName + '  like  "%' + ed_cari.Text +
          '%") limit 0,100', true);
      end;
    3:
      begin
        fungsi.SQLExec(dm.q_cari, 'select ' + clm1.DataBinding.FieldName + ',' +
          clm2.DataBinding.FieldName + ',' + clm3.DataBinding.FieldName +
          ' from ' + nm_tabel + ' where ' + kondisi + '(' + clm1.DataBinding.FieldName
          + ' like "%' + ed_cari.Text + '%" or ' + clm2.DataBinding.FieldName +
          '  like  "%' + ed_cari.Text + '%"  or ' + clm3.DataBinding.FieldName +
          '  like  "%' + ed_cari.Text + '%") limit 0,100', true);
      end;
    4:
      begin
        fungsi.SQLExec(dm.q_cari, 'select ' + clm1.DataBinding.FieldName + ',' +
...
begin
  if cari_kondisi(dm.q_cari.SQL.Text) <> '' then
    kondisi := cari_kondisi(dm.q_cari.SQL.Text) + ' AND '
  else
    kondisi := '';
    ...
    case tipe_cari of
      ...
      66: //global jurnal
        begin
          ...
          else if asal = 'f_jurnal_kas2' then
          begin
          ...
          begin
            ...
            begin
              fungsi.SQLExec(dm.Q_Exe, 'update tb_konfig_akun set kd_akun = ' +
                QuotedStr(dm.q_cari.fieldbyname('kd_kiraan').AsString) +
                ', `update` = date(now()) where no_ix= ' + quotedStr(f_akun_penting.Q_akun_penting.fieldbyname
                ('no_ix').AsString) + '', False);
            end
            else
              exit;
          end;
...
begin
  ...
  begin
    if t_data.DataController.FocusedRecordIndex >= 1 then
      t_data.DataController.FocusedRecordIndex := t_data.DataController.FocusedRecordIndex
        - 1;
    key := VK_NONAME;
  end;
