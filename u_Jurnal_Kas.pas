...
procedure Tf_Jurnal_Kas.sg_jurnalSelectCell(Sender: TObject; ACol,
...
fungsi.SQLExec(dm.Q_temp,'select IFNULL(count(no_ix),0) as jumlah from tb_jurnal_global where refr="'+jenis_jurnal+'"',true);
a:= (dm.Q_temp.fieldbyname('jumlah').AsInteger)+1;

  if a<10       then ed_refrensi.Text:= jenis_jurnal+'-000000'+inttostr(a) else
  if a<100      then ed_refrensi.Text:= jenis_jurnal+'-00000' +inttostr(a) else
  if a<1000     then ed_refrensi.Text:= jenis_jurnal+'-0000'  +inttostr(a) else
  if a<10000    then ed_refrensi.Text:= jenis_jurnal+'-000'   +inttostr(a) else
  if a<100000   then ed_refrensi.Text:= jenis_jurnal+'-00'    +inttostr(a) else
  if a<1000000  then ed_refrensi.Text:= jenis_jurnal+'-0'     +inttostr(a);
...
ed_refrensi.Text+'","'+jenis_jurnal+'","'+floattostr(total_value)+'")',false);

if cb_jenis.ItemIndex = 0 then
begin
fungsi.SQLExec(dm.Q_exe,'insert into tb_jurnal_rinci(kd_perusahaan,ix_jurnal,no_urut,kd_akun, '+
'debet) values ("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)+'",1,"'+ed_no_jurnal.Text+'","'+floattostr(total_value)+'")', false);

  for x:=0 to tableview.DataController.RecordCount-1 do
  begin
  isi_sql:=isi_sql +'("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)
  +'","'+inttostr(x+2)+'","'+TableView.DataController.GetDisplayText(x,0)+'","'+
...
begin
  ...
  begin
  isi_sql:=isi_sql +'("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)
  +'","'+inttostr(x+1)+'","'+TableView.DataController.GetDisplayText(x,0)+'","'+
  floattostr(TableView.DataController.GetValue(x,2))+'"),';
  end;
  delete(isi_sql,length(isi_sql),1);
...
fungsi.SQLExec(dm.Q_exe,'insert into tb_jurnal_rinci(kd_perusahaan,ix_jurnal,no_urut,kd_akun, '+
'kredit) values ("'+f_utama.sb.Panels[3].Text+'","'+inttostr(ix_jurnal)+'","'+inttostr(tableview.DataController.RecordCount+1)+'","'+ed_no_jurnal.Text+'","'+floattostr(total_value)+'")', false);
end;
...
procedure Tf_Jurnal_Kas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  begin
    if CompareStr(dm.Q_temp.fieldbyname('kd_kiraan').AsString,TableView.DataController.GetValue(f,0)) = 0 then
    begin
...
procedure Tf_Jurnal_Kas.ed_codeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=vk_return then
begin
  ...
  fungsi.sqlExec(dm.Q_temp,'select kd_kiraan, n_kiraan from tb_kiraan where kd_kiraan ='+quotedstr(ed_code.Text)+'', true);
...
ed_code.SetFocus;
application.CreateForm(tf_cari, f_cari);
fungsi.SQLExec(dm.q_cari,'select kd_kiraan, n_kiraan from tb_kiraan where kd_kiraan <> '+quotedstr(ed_no_jurnal.Text)+'',true);
f_cari.clm1.caption:='Kode';
f_cari.clm2.caption:='Deskripsi';
u_cari.tipe_cari:=66;
